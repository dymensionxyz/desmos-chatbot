...
async function composeTweet(
    ...
    try {
        ...
        const tweetContent = findTweetContent(tweetContentObject);

        if (!isTweetContent(tweetContent)) {
            elizaLogger.error("Invalid tweet content:", tweetContent);
            return;
        }

        let trimmedContent = tweetContent.text.trim();
...
function findTweetContent(obj: any): any {
    ...
    if ("object" in obj && typeof obj.object === "object") {
        return findTweetContent(obj.object); // Recursively search deeper
    }
